...
def dns_codec(domains, ecs, bufsize, flags=0x0120, RRs=(1, 0, 0, 0), dns_type=1, dns_class=1, trans_type='UDP'):
    '''''
    hostname：解析的域名
    flags：标志位
    RRS：实体数目
...
def dns_sendmsg(domains, host, port, flags, RRs, dns_type, dns_class, trans_type, ecs, bufsize):
    ...
    try:
        ...
        for i in range(1,2):
            ...
            # 发送消息
            sock.sendall(dns_codec(domains, flags=flags, RRs=RRs, dns_type=dns_type, dns_class=dns_class, trans_type=trans_type, ecs=ecs, bufsize=bufsize))
...
if __name__ == '__main__':
    args = parse_get_opt()
    dns_sendmsg(domains=args.domain, host=args.host, port=args.p, flags=args.f, RRs=args.r, dns_type=args.t, dns_class=args.c, trans_type=args.tt, ecs=args.ecs, bufsize=args.bufsize)
